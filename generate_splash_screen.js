...
async function processDeviceImage(deviceInfo, iconImage, backgroundImage) {
  ...
  await new Promise((resolve) => {
    outputStream.on('finish', () => {
      htmlLinks.push(`<link rel="apple-touch-startup-image" href="https://erichsia7.github.io/weave/dist/splash_screen/${fileName}.png" media="(device-width: ${deviceInfo.width}px) and (device-height: ${deviceInfo.height}px) and (-webkit-device-pixel-ratio: ${Math.round(scale)})">`);
      resolve();
    });
  });
}
...
(async () => {
  try {
    ...
    console.endWeaving('Images and HTML code generated successfully!');
  } catch (err) {
